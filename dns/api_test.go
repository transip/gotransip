...
func TestGetDNSSecEntries(t *testing.T) {
	...
	assert.Equal(t, KeyFlagKSK, dns[0].Flags)
	assert.Equal(t, KeyAlgorithmRSASHA512, dns[0].Algorithm)
	...
	assert.Equal(t, KeyFlagZSK, dns[1].Flags)
	assert.Equal(t, KeyAlgorithmECDSAP384SHA384, dns[1].Algorithm)
